...
本项目围绕 OUR 与 SND 的数据关系，构建一套面向污水生化处理、可运行且可审计的决策支持原型。它解决四个连续问题：数据能否使用、模型是否真正跨日期泛化、当前工况能否安全外推、曝气量应如何分步调整。
...
网页自身再分为两个故障安全模式：`public` 是无配置默认值，只使用确定性合成
生成器；`local` 必须同时设置模式与导入开关，才在侧栏渲染 CSV 选择器。本地
脚本把服务绑定到 `127.0.0.1`，导入内容仅保存在当前 Streamlit 会话内存中。
